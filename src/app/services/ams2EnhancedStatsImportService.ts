...
export class AMS2EnhancedStatsImportService {
    ...
    importFile(filePath: string, fileContent: string, serverNameOverride?: string): ImportResult {
        ...
        this.checkPluginVersion(data, result);
    ...
    private buildSteamIdLookup(session: EnhancedSession): Map<number, string> {
        const lookup = new Map<number, string>();
        for (const d of session.drivers) {
            if (d.steamid) lookup.set(d.refid, d.steamid);
        }
        for (const r of session.results) {
            if (r.steamid && !lookup.has(r.refid)) lookup.set(r.refid, r.steamid);
        }
        return lookup;
    }
    ...
    private checkPluginVersion(data: AMS2EnhancedStatsFile, result: ImportResult): void {
        const fileVersion = data.meta?.plugin_version;
        if (!fileVersion) return;
        store.set('lastSeenPluginVersion', fileVersion);
        if (fileVersion < LatestPluginVersion) {
            result.warnings.push(
                `This file was generated by ams2_stats plugin v${fileVersion}. ` +
                `Version ${LatestPluginVersion} is now available with Steam ID and AI driver tracking. ` +
                `Re-install the plugin from the Lua Plugins page to update.`,
            );
        }
    }
